...
p1.centroid.df <- as.data.frame(gCentroid(parcela1, byid = TRUE)) # extract to label parcels
...
ggRGB(TTC08257_modified.stack, r = 1, g = 2, b = 3, maxpixels = 2e+05, stretch="none", geom_raster = TRUE) + 
    ...
    geom_text(data = p1.centroid.df, aes(label = c("1.1", "1.2") ,y = y, x = x), colour = "white") +
    coord_equal() +
    theme_bw()
```
...
p11.VIs <- spectralIndices(TTC08257_modified.stack_p11, green = 3, red=2, nir =1, indices=c("NDVI", "MSAVI2", "GNDVI"))
...
```{r}
cols <- colorRampPalette(c("red", "yellow", "lightgreen"))(length(breaks)-1)
ggR(p11.VIs$GNDVI, geom_raster = TRUE) +
  labs(x="", y="", title= "Parcela 1.1") +
  scale_fill_gradientn(colours=cols,  na.value=NA) + 
  theme_bw() +
  theme(plot.title = element_text(lineheight=.8, face="bold", vjust=1, hjust = 0.5))
...
```{r}
ggsave("figures/parcela1_1_GNDVI.png", 
 plot = last_plot(), # or give ggplot object name as in myPlot,
 width = 5, height = 5, 
 units = "in", # other options c("in", "cm", "mm"), 
 dpi = 300)
...
```{r}
p12.VIs <- spectralIndices(TTC08257_modified.stack_p12, green = 3, red=2, nir =1, indices=c("NDVI", "MSAVI2", "GNDVI"))
breaks <- seq(0, 1, by=0.01)
...
```{r}
ggR(p12.VIs$GNDVI, geom_raster = TRUE) +
  labs(x="", y="", title="Parcela 1.2") +
  scale_fill_gradientn(colours=cols,  na.value=NA) + 
  theme_bw() +
  theme(plot.title = element_text(lineheight=.8, face="bold", vjust=1, hjust = 0.5))  # make title bold and add spac
...
```{r}
ggsave("figures/parcela1_2_GNDVI.png", 
 plot = last_plot(), # or give ggplot object name as in myPlot,
 width = 5, height = 5, 
 units = "in", # other options c("in", "cm", "mm"), 
 dpi = 300)
...
```{r}
ggsave("figures/boxplot_p11_p12_GNDVI.png", 
 plot = last_plot(), # or give ggplot object name as in myPlot,
 width = 5, height = 5, 
 units = "in", # other options c("in", "cm", "mm"), 
 dpi = 300)
...
```{r}
ggsave("figures/histo_p11_p12_NDVI.png", 
 plot = last_plot(), # or give ggplot object name as in myPlot,
 width = 5, height = 5, 
 units = "in", # other options c("in", "cm", "mm"), 
 dpi = 300)
...
```{r}
l.df.VIs.MSAVI2$title <- "MSAVI2" # fake
...
```{r}
ggsave("figures/histo_p11_p12_MSAVI2.png", 
 plot = last_plot(), # or give ggplot object name as in myPlot,
 width = 5, height = 5, 
 units = "in", # other options c("in", "cm", "mm"), 
 dpi = 300)
...
```{r}
ggsave("figures/histo_p11_p12_GNDVI.png", 
 plot = last_plot(), # or give ggplot object name as in myPlot,
 width = 5, height = 5, 
 units = "in", # other options c("in", "cm", "mm"), 
 dpi = 300)
...
```{r}
saveRDS(l.df.VIs, file = "VIP1.rds")
```


```{r}
# save p1 layers
writeRaster(stack(p11.VIs), paste("p11_", names(p11.VIs), sep = ''), bylayer=TRUE, format='GTiff')
writeRaster(stack(p12.VIs), paste("p12_", names(p12.VIs), sep = ''), bylayer=TRUE, format='GTiff')
